...
function [D coupling] = DiffusionMatrixInsideDistr_RBC(n,R,kappa,alpha)
fprintf('*          In DiffusionMatrixInsideDistr_RBC.m *\n')
...
%aux2 = ((rplus(n)/r(n))^2)/(h*alpha); 
aux2 = ((rplus(n)/r(n))^2)*(alpha/h); 
D_R(n+1) = -(aux1 + aux2);
D_R(n)   = aux1;
D_R      = sparse(D_R);
...
fprintf('*          Out DiffusionMatrixInsideDistr_RBC.m *\n')
