...
%
%persistent progfig progdata lastupdate XTick top_val
persistent progfig progdata XTick top_val
...
if nargin > 1 % a reset
    ...
    name=input{1};
    top_val = input{2}; % highest number top=1200
    nd = input{3};  % number decades to gen = 11

    top_pow = floor(log10(top_val)); % 3 from; 10^3 = 1000
    top_dec = 10^top_pow; % - decade below = 1000
    low_pow = top_pow - nd;
    low_dec = 10^low_pow;
    XTick=logspace(low_pow,top_pow,nd+1);

    progfig = [];
    progdata = []; % Forget obsolete data

    ninput = 1; % this was used in original to set multipel bars - revisit if needed
    % maybe check for string as reset and o.w. use set of value for multiples - ??? later
    in_val= 0;

elseif nargin == 1 % get val
    in_val = input{1};
    if in_val == top_val
        if ishandle(progfig)
            delete(progfig) % Close progress bar
        end
        %clear progfig progdata lastupdate XTick top_val % Clear persistent vars
        clear progfig progdata XTick top_val % Clear persistent vars
        drawnow
        return
    end
...
if ishandle(progfig)
    ...
    height = height * heightfactor + 0.01; % DALE add for ticks
    vpad = vpad / heightfactor;
    
    % Initialize progress bar figure (screen position = left,bottom)
    left = (1 - width) / 2;
    bottom = (1 - height) / 2;
    progfig = figure(...
        'Units', 'normalized',...
...
for ndx = 1:ninput
    %if ~isempty(input{ndx})
        progdata(ndx).fractiondone = in_val;%input{ndx};
...
for ndx = 1:length(progdata)
    set(progdata(ndx).progpatch, 'XData', ...
        [XTick(1), progdata(ndx).fractiondone, progdata(ndx).fractiondone, XTick(1)])
end
