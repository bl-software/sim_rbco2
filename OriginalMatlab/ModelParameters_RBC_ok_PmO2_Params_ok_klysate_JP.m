...
% Notes: 
% 20250205: RO is now using the genotype specifc values for kb(1) measured
% by Pz (N = 4, mean 11.60)

% R. Occhipinti 

% 0. Mouse Type

Mouse_Type = 'dKO'; % WT, AQP1-KO, RhAG-KO, dKO
...
if strcmp(Mouse_Type,'WT')
    ...
    Hbtot_in = 18.73; % mM
    kb(1) = 11.60;  % Reaction rate of HbO2 -> Hb + O2  (units: sec^-1) 11.60 is value measured by Pan for WT (N = 4);
    
elseif strcmp(Mouse_Type,'AQP1-KO')
    R = 1.04e-4;   % Radius of the cell in cm 
    Hbtot_in = 17.71; % mM
    kb(1) = 12.31;  % Reaction rate of HbO2 -> Hb + O2  (units: sec^-1) 12.31 is value measured by Pan for AQP1-KO (N = 4);
    
elseif strcmp(Mouse_Type,'RhAG-KO')
    R = 1.09e-4;   % Radius of the cell in cm 
    Hbtot_in = 17.87; % mM
    kb(1) = 12.03;  % Reaction rate of HbO2 -> Hb + O2  (units: sec^-1) 12.03 is value measured by Pan for RhAG-KO (N = 4);
 
elseif strcmp(Mouse_Type,'dKO')
    R = 1.07e-4;   % Radius of the cell in cm 
    Hbtot_in = 18.16; % mM
    kb(1) = 10.44;  % Reaction rate of HbO2 -> Hb + O2  (units: sec^-1) 10.44 is value measured by Pan for dKO (N = 4);
